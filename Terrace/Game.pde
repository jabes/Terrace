  ...
  */
  ...
  void init (int[][] newInteractiveObjectData, int[][] newEnemyData) {
    ...
    if (enemyData != null && enemyData.length > 0) {
      for (int i = 0, ii = enemyData.length; i < ii; i++) {
        if (enemyData[i][2] == 1) {
          enemies.add(new GilliamKnight(enemyData[i][0], enemyData[i][1]));
        } else if (enemyData[i][2] == 2) {
          enemies.add(new Kintot(enemyData[i][0], enemyData[i][1]));
        }
      }
    }
  ...
  void iterate () {
    ...
    for (int i = 0, ii = enemies.size(); i < ii; i++) {
      ...
      if (enemy.isAlive && player.isTouching(enemy.posX, enemy.posY, enemy.sizeWidth, enemy.sizeHeight)) {
        player.destroy();
      }
    }
