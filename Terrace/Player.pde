public class Player extends Hitbox {
  ...
  public void iterate () {
    ...
    */
    ...
    // moving down
    if (speedY >= 0) {
      movableDown  = !touchingBottomTile && !touchingViewportBottom;
    }

    // moving up
    if (speedY <= 0) {
      movableUp = !touchingTopTile && !touchingViewportTop;
    }
  ...
  private void movePlayer (float x, float y) {
    ...
    if (newDirection == 1) { // RIGHT
      if (isJumping) {
        if (isShooting) {
          animatePlayer = animationShootJumpRight;
        } else {
          animatePlayer = animationJumpRight;
        }
      } else {
        if (isShooting) {
          animatePlayer = animationShootWalkRight;
        } else {
          animatePlayer = animationWalkRight;
        }
      }
    } else if (newDirection == 3) { // LEFT
      if (isJumping) {
        if (isShooting) {
          animatePlayer = animationShootJumpLeft;
        } else {
          animatePlayer = animationJumpLeft;
        }
      } else {
        if (isShooting) {
          animatePlayer = animationShootWalkLeft;
        } else {
          animatePlayer = animationWalkLeft;
        }
      }
    }
    ...
    if (speedX == 0 && !isJumping) {
      animatePlayer.getFrame(0);
    } else {
      animatePlayer.run();
    }
    ...
    if (gunInterval == 0) {
      for (int i = 0, ii = globals.maxBullets; i < ii; i++) {
        ...
        if (!game.bullets[i].isActive) {
          if (dir == 1) {
            game.bullets[i].init(super.posX + super.sizeWidth, super.posY + halfHeight, dir);  // RIGHT
          } else if (dir == 3) {
            game.bullets[i].init(super.posX, super.posY + halfHeight, dir);  // LEFT
          }
